class Player {
    constructor(spr, health, mana, gameScene, controlType) {
        ...
        //Assign inventory as blank first
        ...
        //Sprite, collider and spawnpoint info
    ...
    GetGearIndex(index) {
        //Return equipment position in the UI
    ...
    RemoveGear(index, item) {
        ...
        let e = item.parent;
        //Find inventory item, put removed item there and delete the gear item instance
        for (let i = 0; i < this.inventoryItems.length; i++) {
            if (this.inventoryItems[i] == null) {
                ...
                //Move item to inventory and create a copy there
    ...
    {
        //Move player between maps or respawn
        this.characterSprite.x = posX;
        this.characterSprite.y = posY;
        //Move players worn items too
        if(this.mainWeapon){
    ...
    {
        //Move function for normal movement
        ...
        //Move gear with the player
        this.MoveGear(this.mainWeapon);
        this.MoveGear(this.shield);

        //hide the chat box prompt if the character is not near a talking NPC
    ...
    TakeDamage(amount)
    {
        //Damage register and then apply red tint to indicate damage
    ...
    Destruct()
    {
        //Destruct function along with reset functionality for respawning
        this.health = 100;
        this.mana = 100;
    ...
    {
        //Set items the same velocity as the player to prevent problems with hitboxes
        if(gearItem) {
    ...
    interact()
    {
        //If colliding with an interactable NPC, activate their chat in the first instance
    ...
    positionInteractMenu(collisionPartner)
    {
        ...
        this.UI.notificationBoxText.setScrollFactor(0);
        this.UI.notificationBoxText.visible = true;
    ...
    Update(cursors){
        ...
        //Check damage timer before clearing the red tint
        //Gives the appearance of being present for a split second

class UI {
    constructor(players, quests, gameScene){
        //UI players reference and initialise currently selected player
        ...
        //Default quest scripts (unassigned)
    ...
    InitialiseHUD() {
        ...
        this.chatboxCloseButton = this.gameScene.add.sprite(737, 421, 'chatboxclosebutton', 1).setInteractive().setDepth(2).setScrollFactor(0).setName("chatboxclosebutton");
    ...
    InitialiseInventory() {
        //Setup player inventory with default items
    ...
    ChangePlayer(index){
        //Change player to show their respective inventories
        if(index == 0){
    ...
    ToggleChatBox(isOpen) {
        //Toggle the chatbox sprite being open
        if (isOpen) {
    ...
    ActivateQuestBox(index) {
        //Quest box shows current progress in each quest with a brief description of current mission
        this.notificationBoxText.setVisible(true).setDepth(3).setScrollFactor(0);
    ...
    ToggleNotificationBox(isOpen, notificationText) {
        //Toggle notification box elements, same idea as chat box with same implementation
        if (isOpen) {
    ...
    CheckInput() {
        //Cycle through all the minor and miscallaneous UI buttons found throughout
        //the game. This function is found in the create method of the phaser instance
        let self = this;
        this.gameScene.input.on('gameobjectdown', function (pointer, button) {
            ...
            else if (button.name === 'chatboxcontinuebutton') self.ActivateChat(self.playerCollisionPartner.information, self.chatIndex + 1, false, null);
            ...
            for(let j = 0; j < players.length; j++) {
                if (players[j].collisionPartner != null) {
                    if (button === players[j].collisionPartner.ShopBackgroundCloseButton) {
                        ...
                        //Cycle through shop items by their sprites if player is in a shop menu

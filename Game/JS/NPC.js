class NPC{
    constructor(spr, rot, info, infoFork, gamescene, shopType, cbd, xr, yr, players, isranged, givesQuestInfo, questIndex, questPlace, cashdrop) {
        ...
        //Shop attributes (if applicable)
        this.shopItems = [];
        this.ShopBackgroundCloseButton = null;

        //Quest attributes (if applicable)
        ...
        //Assign predefined targets via their colliders
        let self = this;
    ...
    InitialiseShop(shopType)
    {
        ...
        for(let i = 1; i < itemArray.length -7; i+=8)
        {
            ...
            //Add items in grid fashion across the screen
            ...
            //Assign name and caption text and add to shopitems container
        ...
        //Keep shop closed until prompted
        this.ToggleShop(false);
    }

    ToggleShop(isOpen)
    {
        //Activate all shop sprites if prompted, otherwise close all shop sprites and deactivate
        //interactivity
        if(isOpen)
        {
    ...
    TakeDamage(amount)
    {
        //Simple damage tint effect, turns player red for a split second when registering damage
        this.health -= amount;
        this.characterSprite.setTint('0xff0000', '0xff0000', '0xff0000', '0xff0000');
        this.damageTimer = 0.25;
    }

    //To check if facing the destination or player
    ...
    //Attack function, applies damage and kills enemy if applicable
    ...
    //Basic move function based on pathfinding
    Move(rot, velx, vely)
    {
        this.characterSprite.rotation = rot;
    ...
    Update() {
        ...
        //Update any projectiles currently fired
        for (let i = 0; i < this.projectiles.length; i++)
            if (this.projectiles[i] != null) {
                this.projectiles[i].Update();
            }

        //If alive, update everything
        if (this.isAlive) {
            //Melee attack timer
            ...
            if (this.xRange > 0 || this.yRange > 0) {
                ...
                if (this.moveDirection == 1 || this.moveDirection == 2) {
                    ...
                    //If not moving, assign everything to 0
